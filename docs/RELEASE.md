...
| :---: | :--- | :--- | :--- |
| 1 | `debug_kit` | 0.8.1 | Core — publish first, no local deps |
| 2 | `debug_kit_dio` | 0.4.0 | Depends on `debug_kit` |
...
- The published `pubspec.yaml` uses a proper hosted version constraint (`^0.8.1` for `debug_kit`, `^0.4.0` for `debug_kit_dio`), not a `path:` dependency.
...
```bash
git tag debug_kit-v0.8.1
git tag debug_kit_dio-v0.4.0
git tag debug_kit_go_router-v0.2.2
git tag debug_kit_riverpod-v0.2.2
git push --tags
```

GitHub release title for this core release:

`DebugKit 0.8.1 — Compact Network Inspector Layout`
